...
上训练网络，损失是原始力矩回归（目标值为未归一化的原始力矩 $u_t$）：
...
$$
\hat u_t = K_p(x_t)\odot\Delta q + K_d(x_t)\odot\Delta v + \tau_{\mathrm{ff}}(x_t),
$$

其中 $\Delta q = q_t^{\mathrm{ref}} - q_t$，$\Delta v = \dot q_t^{\mathrm{ref}} - \dot q_t$，$K_p, K_d > 0$（通过 softplus 保证正定性），$\tau_{\mathrm{ff}}$ 为可学习前馈项。网络以 $(x_t, x_{t+1}^{\mathrm{ref}})$ 为输入，输出 $3n_q$ 维向量 $(K_p, K_d, \tau_{\mathrm{ff}})$。

### 4.4 分解动力学架构

刚体逆动力学方程 $\tau = M(q)\ddot q + C(q,\dot q)\dot q + g(q)$ 在加速度上是**线性的**。
利用此结构，将网络分解为：

$$
\hat u_t = A(q_t,\dot q_t)\cdot\begin{bmatrix}\Delta q\\ \Delta v\end{bmatrix} + b(q_t,\dot q_t),
$$

其中 $A(q,\dot q)\in\mathbb{R}^{n_q\times 2n_q}$ 是由当前状态预测的全增益矩阵（类比质量矩阵 $M(q)$），$b(q,\dot q)\in\mathbb{R}^{n_q}$ 是状态相关偏置（类比重力和科氏项）。两个子网络仅接受当前状态 $x_t$，跟踪误差 $[\Delta q; \Delta v]$ 通过线性乘法引入，与物理方程的结构完全一致。
...
proposal 进一步利用“仿真器就是精确前向模型”这一优势。
